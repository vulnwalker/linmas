...
class PembinaanData implements FromQuery, WithTitle, WithHeadings, WithEvents
{
    ...
    if (!empty($tanggal_selesai)){
        ...
        $sampai = date($tanggalSelesaiExplode[2].'-'.$tanggalSelesaiExplode[0].'-'.$tanggalSelesaiExplode[1]);
          ...
          if (!empty($this->nama)) $query->where('pembinaans.nama','LIKE', "%$nama%");
          if (!empty($this->penyelengara)) $query->where('pembinaans.penyelengara','LIKE', "%$penyelengara%");
          if (!empty($this->lokasi)) $query->where('pembinaans.lokasi','LIKE', "%$lokasi%");
          if (!empty($this->kegiatan)) $query->where('pembinaans.kegiatan','LIKE', "%$kegiatan%");
          if (!empty($this->tanggal_mulai) && empty($tanggal_selesai)){
